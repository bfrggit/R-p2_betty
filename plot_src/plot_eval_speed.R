...
cat("Processing...", "\n")
num_loops = 5L
line_cols = c("speed", "nact", "cover", "util", "overall")
...
dm = melt(df[, line_cols], id.vars = "speed", variable.name = "series")
dm_se = melt(df_se[, line_cols], id.vars = "speed", variable.name = "series")
dm$se = dm_se$value
plot_obj = ggplot(data = dm, aes(x = speed)) +
    ...
    geom_area(
        ...
        aes(
            ...
            ymax = value + se,
            color = series
        ), size = 0.5, width = (max(df$speed) - min(df$speed)) * 0.02
    ) + geom_point(
