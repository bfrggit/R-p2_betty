...
create_placement_rand <<- function(
    ...
    dimnames(placement_mat_rand_roll)[[1]] <<- z_nd_str("n", val_n)
    dimnames(placement_mat_rand_roll)[[2]] <<- z_nd_str("c", val_m)

    stopifnot(val_n == dim(placement_mat_rand_roll)[1])
    stopifnot(val_m == dim(placement_mat_rand_roll)[2])
    stopifnot(duration_frames + 1L == dim(placement_mat_rand_roll)[3])
...
update_placement_rand <<- function(t_frame, simu_n) {
    ...
    for(jnd in 1:val_n) {
        placement_mat_rand_roll[jnd, placement_per_node[jnd], simu_n + 1L] <<- 1
    }
